...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument("--cache",type=Path,required=True); ap.add_argument("--pairs",type=Path,required=True); ap.add_argument("--geometry",type=Path,required=True); ap.add_argument("--confidence",type=Path,required=True); ap.add_argument("--output",type=Path,required=True); ap.add_argument("--fixed-temperature",type=float); a=ap.parse_args()
 ...
 opt.step(closure) if a.fixed_temperature is None else None; temperature=float(a.fixed_temperature if a.fixed_temperature is not None else temp.detach().clamp_min(.05)); test=np.arange(n) if a.fixed_temperature is not None else test; rawp=1/(1+np.exp(-z[test])); calp=1/(1+np.exp(-z[test]/temperature)); report={"schema_version":1,"stage":"latent confidence probability calibration","label":"projected-valid under sensor GT pose; calibration-only label, not semantic correctness","calibration_pairs":int(len(ds)//3) if a.fixed_temperature is None else 0,"test_pairs":int(len(ds)-len(ds)//3) if a.fixed_temperature is None else len(ds),"temperature":temperature,"raw_test":metrics(rawp,y[test]),"calibrated_test":metrics(calp,y[test]),"scope":"post-hoc temperature scaling with strict held-out split; fixed-temperature mode is zero-shot calibration transfer"}
